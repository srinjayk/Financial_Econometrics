...
for x in range(row_count):
	...
	for company_name in WB:
		wbreturnarr.append(companies[company_name]['ROI'][x])
	for company_name in WS:
		wsreturnarr.append(companies[company_name]['ROI'][x])
	for company_name in LB:
		lbreturnarr.append(companies[company_name]['ROI'][x])
	for company_name in LS:
		lsreturnarr.append(companies[company_name]['ROI'][x])
	...
	if len(wbreturnarr)>0:
		wbreturn = np.average(wbreturnarr)
	else:
		wbreturn = 0

	if len(wsreturnarr)>0:
		wsreturn = np.average(wsreturnarr)
	else:
		wsreturn = 0

	if len(lbreturnarr)>0:
		lbreturn = np.average(lbreturnarr)
	else:
		lbreturn = 0

	if len(lsreturnarr)>0:
		lsreturn = np.average(lsreturnarr)
	else:
		lsreturn = 0
